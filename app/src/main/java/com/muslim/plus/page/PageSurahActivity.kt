...
import com.muslim.plus.MainViewModel
import com.muslim.plus.R
...
class PageSurahActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (checkNett.checkInternetAccess()) {
            ...
            val fragment =
                mFragmentManager.findFragmentByTag(ConnectionStateFragment::class.java.simpleName)
            if (fragment !is ConnectionStateFragment) {
                mFragmentManager.beginTransaction().apply {
                    add(
                        R.id.frame_connection_content,
                        mConnectionStateFragment,
                        ConnectionStateFragment::class.java.simpleName
                    )
                    commit()
                }
            }
